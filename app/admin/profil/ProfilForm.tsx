...
export function ProfilForm({ profil }: { profil: ProfilInput & { id: string } }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl bg-card p-6 rounded-lg border shadow-sm">
      <div className="space-y-4">
        <div>
          <label className="text-sm font-medium mb-1 block">Sambutan Lurah</label>
          <Textarea 
        ...
        <div>
          <label className="text-sm font-medium mb-1 block">URL Foto Lurah</label>
          <Input 
        ...
        <div>
          <label className="text-sm font-medium mb-1 block">Sejarah Singkat Kelurahan</label>
          <Textarea 
            value={form.sejarah} 
      ...
      <div className="border-t pt-4">
        <h3 className="text-lg font-semibold mb-4">Data Demografi & Wilayah</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <label className="text-sm font-medium mb-1 block">Jumlah Penduduk</label>
            <Input type="number" value={form.jumlah_penduduk} onChange={(e) => setForm({ ...form, jumlah_penduduk: parseInt(e.target.value) || 0 })} />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">Jumlah KK</label>
            <Input type="number" value={form.jumlah_kk} onChange={(e) => setForm({ ...form, jumlah_kk: parseInt(e.target.value) || 0 })} />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">Jumlah RT</label>
            <Input type="number" value={form.jumlah_rt} onChange={(e) => setForm({ ...form, jumlah_rt: parseInt(e.target.value) || 0 })} />
          </div>
          <div>
            <label className="text-sm font-medium mb-1 block">Jumlah RW</label>
      ...
      <div className="border-t pt-4">
        <label className="text-sm font-medium mb-1 block">Google Maps Embed URL (Iframe src)</label>
        <Input
