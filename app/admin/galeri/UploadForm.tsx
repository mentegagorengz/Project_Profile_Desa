...
export function UploadForm() {
  ...
  return (
    <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 items-end rounded-lg border border-border p-4 bg-card shadow-sm">
      <div className="w-full sm:flex-1">
        <label className="text-sm font-medium text-foreground mb-1 block">Pilih Foto</label>
        <Input 
          type="file" 
          name="file" 
          accept="image/*" 
          required 
          className="cursor-pointer border-border focus:border-ring" 
        />
      </div>
      <div className="w-full sm:flex-1">
        <label className="text-sm font-medium text-foreground mb-1 block">Caption (Opsional)</label>
        <Input 
          type="text" 
          name="caption" 
      ...
      <Button 
        type="submit" 
        disabled={uploading} 
        className="w-full sm:w-auto bg-primary hover:bg-primary/90 text-primary-foreground font-display"
      >
        {uploading ? (
          'Mengunggah...'
        ) : (
