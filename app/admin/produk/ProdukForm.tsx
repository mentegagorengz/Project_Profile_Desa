...
export function ProdukForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-3 items-end rounded-lg border border-border p-4 bg-card shadow-sm">
      <div className="flex-1 w-full">
        <label className="text-sm font-medium text-foreground mb-1 block">Nama Produk</label>
        <Input value={nama} onChange={(e) => setNama(e.target.value)} required className="border-border focus:border-ring" />
      </div>
      <div className="flex-1 w-full">
        <label className="text-sm font-medium text-foreground mb-1 block">Kategori</label>
        <Input value={kategori} onChange={(e) => setKategori(e.target.value)} required className="border-border focus:border-ring" />
      </div>
      <div className="flex-1 w-full">
        <label className="text-sm font-medium text-foreground mb-1 block">Harga/kg (Rp)</label>
        <Input
      ...
      <Button type="submit" disabled={loading} className="w-full md:w-auto bg-primary hover:bg-primary/90 text-primary-foreground font-display">
...
export function ProdukRow({ produk }: { produk: Produk }) {
  ...
  if (editing) {
    return (
      <div className="rounded-lg border border-ring bg-card p-4 flex flex-col md:flex-row gap-3 items-end shadow-sm">
        <div className="flex-1 w-full">
          <label className="text-xs font-medium text-foreground mb-1 block">Nama Produk</label>
          <Input value={form.nama_produk} onChange={(e) => setForm({ ...form, nama_produk: e.target.value })} className="border-border focus:border-ring" />
        </div>
        <div className="flex-1 w-full">
          <label className="text-xs font-medium text-foreground mb-1 block">Kategori</label>
          <Input value={form.kategori} onChange={(e) => setForm({ ...form, kategori: e.target.value })} className="border-border focus:border-ring" />
        </div>
        <div className="flex-1 w-full">
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 flex items-center justify-between gap-4 hover:border-primary/40 hover:shadow-sm transition-all duration-200">
      <div className="flex-1 min-w-0">
        <p className="font-medium text-foreground">{produk.nama_produk}</p>
        <p className="text-xs text-primary">{produk.kategori}</p>
      </div>
      <p className="font-mono text-foreground font-semibold text-sm shrink-0">
